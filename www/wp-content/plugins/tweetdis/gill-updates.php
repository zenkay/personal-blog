...
function gill_updates_exclude_tweet_dis( $r, $url ) {
...
add_filter( 'http_request_args', 'gill_updates_exclude_tweet_dis', 5, 2 );


//Returns current plugin info.
function gill_plugin_get_tweet_dis($i) {
	global $this_file;
...
register_activation_hook($this_file, 'gill_check_activation_tweet_dis');

function gill_check_activation_tweet_dis() {
	wp_schedule_event(time(), 'twicedaily', 'gill_check_event');
}
function gill_check_update_tweet_dis() {
	global $wp_version;
	global $this_file;
	global $update_check;
	...
	if(gill_plugin_get_tweet_dis("Version") == $version) return false;
	$plugin_transient = get_site_transient('update_plugins');
	$a = array(
		'slug' => $plugin_folder,
		'new_version' => $version,
		'url' => gill_plugin_get_tweet_dis("AuthorURI"),
		'package' => $url
	);
	$o = (object) $a;
